...
// Get the month number (0-based index, so add 1)
const monthNumber = date.getMonth() + 1;

// Get the current year
const year = date.getFullYear();

// Calculate the percentage of the year relative to the first day of the current month
const startOfYear = new Date(year, 0, 1); // January 1st of the current year
const startOfMonth = new Date(year, date.getMonth(), 1); // 1st day of the current month
...
const endTime = endOfYear.getTime();

const yearPercentage = ((monthStartTime - startTime) / (endTime - startTime)) * 100;
const formattedPercentage = yearPercentage.toFixed(2); // Format to 2 decimal places
