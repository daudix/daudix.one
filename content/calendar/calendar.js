window.calendar = (function () {
  ...
  function makeEle(name, attrs) {
    ...
    for (var i in attrs)
      if (attrs.hasOwnProperty(i)) ele.setAttribute(i, attrs[i]);
    return ele;
  }
  ...
  class Calendar {
    constructor(elem, opts) {
      ...
      this.selectedDates = [];
      this.elem = elem;
      this.disabledDates = (opts.disabledDates || []).map((d) =>
        this.formatDateMMDDYY(d),
      );
      ...
      this.events = [];
      this.month = opts.hasOwnProperty("month")
        ? opts.month - 1
        : new Date().getMonth();
      this.year = opts.hasOwnProperty("year")
        ? opts.year
      ...
      for (var i = opts.events.length; i--; )
        this.addEvent(opts.events[i], false);
    ...
    async addEvent(evt, draw = true) {
      ...
      if (hasDesc && (hasDate || (hasStartDate && hasEndDate))) {
        ...
        throw new Error(
          "All events must have a 'desc' property and either a 'date' property or a 'startDate' and 'endDate' property",
        );
    ...
    clearSelection() {
      ...
      for (let i = active.length; i--; )
        active[i].classList.remove("cjs-active");
      this.selectedDates = [];
      return this;
    }
    ...
    selectDate(date) {
      ...
      this.elem
        .getElementsByClassName("cjs-dayCell" + date.getDate())[0]
    ...
    getSelection() {
      ...
      for (let i = 0; i < this.selectedDates.length; i++)
        sel.push(
          new Date(
            this.selectedDates[i].year,
            this.selectedDates[i].month,
            this.selectedDates[i].day,
          ),
        );
      return sel;
    }
    ...
    async drawCalendar() {
      ...
      // draw title bar
      var year = this.abbrYear
          ? "'" + ("" + this.year).substr(2, 2)
          : this.year,
        monthArrayName = !this.abbrMonth ? "monthsFull" : "monthsAbbr",
        monthName = this[monthArrayName][this.month],
        lastMo = this.monthsAbbr[this.month - 1]
          ? this.monthsAbbr[this.month - 1]
          : this.monthsAbbr[11],
        nextMo = this.monthsAbbr[this.month + 1]
          ? this.monthsAbbr[this.month + 1]
          : this.monthsAbbr[0];
      this.elem.insertAdjacentHTML(
        "beforeend",
        "<div class='cjs-weekRow cjs-calHeader'><button class='cjs-lastLink'>&#8592; " +
          lastMo +
          "</button><button class='cjs-nextLink'>" +
          nextMo +
          " &#8594;</button></div>",
      );

      // My shit starts here
      document.getElementById("month-number").textContent = String(
        this.month + 1,
      ).padStart(2, "0");
      ...
      document
        ...
        .style.setProperty("--year-percentage", `${percentage}%`);
      document.getElementById("progress-month").textContent =
        `${String(this.month + 1).padStart(2, "0")}/12`;
      // My shit ends here
      ...
      for (let i = 0; i < this[dayArrayName].length; i++) {
        ...
        dayNames.insertAdjacentHTML(
          "beforeend",
          "<div class='cjs-dayCol cjs-left cjs-top cjs-bottom cjs-dayHeader" +
            directionalClass +
            "'><div class='cjs-dayHeaderCell'>" +
            this[dayArrayName][i] +
            "</div></div>",
        );
      }
      ...
      while (currentDate <= lastDate) {
        ...
        var week = makeEle("div", {
          class: "cjs-weekRow calweekid" + currentWeek,
        });
        // draw days before the 1st of the month
        while (currentDate === 1 && currentDay < firstDayofWeek) {
          week.insertAdjacentHTML(
            "beforeend",
            "<div class='cjs-dayCol cjs-blankday cjs-bottom cjs-left'><div class='cjs-dayContent'><div class='cjs-dayTable'><div class='cjs-dayCell'></div></div></div></div>",
          );
          currentDay++;
        }
        // Store the events
        ...
        while (currentDay < 7 && currentDate <= lastDate) {
          ...
          for (var n = 0; n < this.events.length; n++) {
            if (!this.events[n].hasOwnProperty("date")) {
              ...
              if (yearMatches && monthMatches && dayMatches) {
                if (!dayEvents.hasOwnProperty(n))
                  dayEvents[n] = {
                    event: this.events[n],
                    startdayid: currentDate,
                    enddayid: currentDate,
                    start: currentDay,
                    end: currentDay,
                    eventid: n,
                  };
          ...
          var isDisabled = this.disabledDates.includes(
            `${this.month + 1}/${currentDate}/${this.year}`,
          );
          ...
          if (lastDate - currentDate < 7) {
            ...
            if (currentDay === 6 && currentDate === lastDate)
              directionalClass += " cjs-bottom-right";
          }
          // draw day
          week.insertAdjacentHTML(
            "beforeend",
            "<div class='cjs-dayCol cjs-bottom cjs-left" +
              directionalClass +
              " " +
              (isDisabled ? "cjs-blankday" : "") +
              "'><div class='cjs-dayContent'><div class='cjs-dayTable'><div class='cjs-dayCell " +
              (isDisabled ? "" : "cjs-calDay") +
              " cjs-dayCell" +
              currentDate +
              "' data-day='" +
              currentDate +
        ...
        while (currentDay < 7) {
          ...
          week.insertAdjacentHTML(
            "beforeend",
            "<div class='cjs-dayCol cjs-blankday cjs-bottom" +
              directionalClass +
              "'><div class='cjs-dayContent'><div class='cjs-dayTable'><div class='cjs-dayCell'></div></div></div></div>",
          );
          currentDay++;
        }
        ...
        for (let eid in weekEvents) {
          ...
          if (!weekEvents[eid].hasOwnProperty("start")) {
            ...
            weekEvents[eid].startdayid =
              weekEvents[eid].enddayid - weekEvents[eid].end;
          }
          weekEvents[eid].week = currentWeek;
          var egid = this._eventGroups.length;
          this._eventGroups.push(weekEvents[eid]);
          var cls =
            undefined === this.events[eid].type
              ? ""
              : " " + this.events[eid].type;
          var desc = this.events[eid].desc;
          if (this.ellipse)
            ((desc = "<span>" + desc + "</span>"), (cls += " cjs-ellipse"));
          var evt =
            "<div class='cjs-calEvent" +
            egid +
            " cjs-calEvent" +
            cls +
            "'  data-eventid='" +
            this._eventGroups[egid].eventid +
            "'>" +
            desc +
            "</div>";
      ...
      for (var i = wks.length; i--; )
        wks[i].insertAdjacentHTML(
          "beforeend",
          "<div class='cjs-clearfix'></div>",
        );
    ...
    _positionEventGroups() {
      ...
      for (let i = 0; i < this._eventGroups.length; i++) {
        let e = this._eventGroups[i],
          ...
          dayCellHt = this.elem
            .getElementsByClassName("cjs-dayCell")[0]
            .getBoundingClientRect().height,
          dayColHt = this.elem
            .getElementsByClassName("cjs-dayCol")[7]
            .getBoundingClientRect().height,
          dayColWd = this.elem
            .getElementsByClassName("cjs-dayCol")[7]
            .getBoundingClientRect().width,
          dateLabelHeight = this.elem
            .getElementsByClassName("cjs-dateLabel")[0]
            .getBoundingClientRect().height,
        ...
        if (eventBottom > bottomBorder) {
          ...
          if (lastPositAvailable === false) {
            ...
            let evt =
              ...
              desc +
              "</div>";
            let week = this.elem.getElementsByClassName(
              "calweekid" + e.week,
            )[0];
            week.insertAdjacentHTML("beforeend", evt);
            for (let n = e.startdayid; n <= e.enddayid; n++)
      ...
      for (let date in otherEvents) {
        ...
        d.insertAdjacentHTML(
          "beforeend",
          '<div class="cjs-otherEvents">+' + otherEvents[date] + "</div>",
        );
      }
      return this;
    }
