...
let achievementBasicScience =
  JSON.parse(localStorage.getItem("achievementBasicScience")) || false; // Track if achievement has been triggered
...
const saveAchievementBasicScience = () => {
  localStorage.setItem(
    "achievementBasicScience",
    JSON.stringify(achievementBasicScience),
  );
};
...
checkboxIds.forEach((id) => {
  ...
  checkbox.addEventListener("change", () => {
    ...
    if (allToggledOnce() && !achievementBasicScience) {
      ...
      achievementBasicScience = document.getElementById(
...
document.addEventListener("keydown", function (pEvent) {
  if ((pEvent.ctrlKey || pEvent.metaKey) && pEvent.key === "p") {
    if (localStorage.rocketScienceAchieved !== "true") {
      ...
      const achievementRocketScience = document.getElementById(
        "achievement-rocket-science",
      );
